              ...
              <View style={ncm.iconGrid}>
                {NEW_CAT_ICONS.map(ic => (
                  <TouchableOpacity
                    key={ic}
                    style={[ncm.iconOpt, {
                      backgroundColor: icon === ic ? '#1B7A4A' : theme.surface,
                      borderColor: icon === ic ? '#1B7A4A' : theme.inputBorder,
                    }]}
                    onPress={() => setIcon(ic)}
                    activeOpacity={0.7}
                  >
    ...
    >
      ...
      <TouchableOpacity style={{ flex: 1 }} onPress={() => onEditIncome(inc.id)} activeOpacity={0.85}>
        ...
        <Text style={[s.incMeta, { color: theme.textMuted }]}>
          {fmt12h(inc.time)} · {fmtDateShort(inc.date)}{inc.isRecurring ? ` · ${inc.frequency}` : ''}
        </Text>
      </TouchableOpacity>
