...
function groupByMonth(items: ExpenseEntry[]) {
  const map = new Map<
    string,
    { key: string; label: string; items: ExpenseEntry[] }
  >();
  const sorted = [...items].sort((a, b) => b.date.localeCompare(a.date));
  for (const exp of sorted) {
    const key = monthYearKey(exp.date);
    if (!map.has(key))
      map.set(key, { key, label: monthLabel(exp.date), items: [] });
...
const bh = StyleSheet.create({
  ...
  owlBubble: {
    ...
    maxWidth: 155,
    marginBottom: Math.round(BH_OWL_H * 0.28),
    marginLeft: 6,
    shadowColor: "#000",
...
function FormHeader({
  ...
  return (
    <View style={[fh.wrap, { backgroundColor: theme.headerBg }]}>
      <View style={fh.nav}>
        <TouchableOpacity
          style={[fh.iconBtn, { backgroundColor: theme.iconBtnBg }]}
          onPress={onBack}
          activeOpacity={0.8}
        >
  ...
  return (
    <Modal visible={visible} transparent animationType="slide">
      <Pressable style={pk.overlay} onPress={onClose}>
        <Pressable
          style={[pk.sheet, { backgroundColor: theme.modalBg }]}
          onPress={() => {}}
        >
            ...
            {options.map((opt) => (
              ...
              >
                {renderItem ? (
                  ...
                  <Text style={[pk.itemTxt, { color: theme.textPrimary }]}>
                    {opt}
                  </Text>
                )}
                {selected === opt && (
                  <Ionicons name="checkmark" size={18} color="#1B7A4A" />
                )}
              </TouchableOpacity>
            ))}
          </ScrollView>
    ...
    <>
              ...
              <View style={s.grid}>
                {active.map((cat) => {
                  const count = expenses.filter(
                    (e) => e.categoryId === cat.id && !e.isArchived,
                  ).length;
                  return (
            ...
            <>
              ...
              {archived.map((cat, idx) => (
                <View
                  ...
                  style={[
                    s.archiveRow,
                    idx < archived.length - 1 && s.archiveRowBorder,
                ...
                >
                  ...
                  <Text
                    style={[s.expTitle, { flex: 1, color: theme.textPrimary }]}
                  >
                    {cat.label}
                  </Text>
                  <TouchableOpacity
      ...
      <Modal visible={kebabCatId !== null} transparent animationType="fade">
        <Pressable style={s.kebabOverlay} onPress={() => setKebabId(null)}>
          <Pressable
            style={[s.kebabSheet, { backgroundColor: theme.modalBg }]}
            onPress={() => {}}
          >
            {kebabCat && (
              <>
                <View style={s.kebabHeader}>
                  ...
                  <Text style={[s.kebabCatName, { color: theme.textPrimary }]}>
                    {kebabCat.label}
                  </Text>
                </View>
                <View
                  style={[s.kebabDivider, { backgroundColor: theme.divider }]}
                />
                <TouchableOpacity
                  style={s.kebabItem}
                  onPress={() => {
                    setKebabId(null);
                    onEditCategory(kebabCatId!);
                  }}
                  activeOpacity={0.8}
                >
                  <Ionicons name="create-outline" size={20} color="#1B7A4A" />
                  <Text style={[s.kebabItemTxt, { color: theme.textPrimary }]}>
                    Edit
                  </Text>
                </TouchableOpacity>
                {kebabCat.isArchived ? (
                  <>
                    ...
                    >
                      <Ionicons
                        ...
                        color="#3ECBA8"
                      />
                      <Text style={[s.kebabItemTxt, { color: "#3ECBA8" }]}>
                        Restore
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      <Ionicons
                        ...
                        color="#E05858"
                      />
                      <Text style={[s.kebabItemTxt, { color: "#E05858" }]}>
                        Delete Permanently
                      </Text>
                    </TouchableOpacity>
                  </>
...
function CategoryDetailScreen({
  ...
  const kebabExp = kebabExpId
    ? catExpenses.find((e) => e.id === kebabExpId)
    : null;

  const bodyAnim = useEntranceAnim();
  const addBtnScale = useSharedValue(1);
  const addBtnAnim = useAnimatedStyle(() => ({
  ...
  const renderExpRow = (
    ...
    >
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.85}
      >
        <Text style={[s.expTitle, { color: theme.textPrimary }]}>
          {exp.title}
        </Text>
        <Text style={[s.expMeta, { color: theme.textMuted }]}>
          {fmt12h(exp.time)} · {fmtDateShort(exp.date)}
  ...
  return (
    <>
      ...
      />
      <Animated.View
        style={[s.white, { flex: 1, backgroundColor: theme.cardBg }, bodyAnim]}
      >
        <SlideTabBar
          tabs={["Active", "Archived"]}
          active={tab}
              ...
              {activeGrouped.map((group) => (
                <View key={group.key} style={{ marginBottom: 8 }}>
                  <View style={s.monthHeader}>
                    <Text style={[s.monthLabel, { color: theme.textPrimary }]}>
                      {group.label}
                    </Text>
                    <Ionicons
                  ...
                  {group.items.map((exp, idx) =>
                    renderExpRow(exp, idx, group.items),
                  )}
                </View>
              ))}
            </>
          ...
          <View style={s.addBtnWrapper}>
            <Animated.View style={addBtnAnim}>
              <TouchableOpacity
                style={s.addBtn}
                onPressIn={() => {
                  addBtnScale.value = withSpring(0.96, {
                    damping: 15,
                    stiffness: 300,
                  });
                }}
            ...
            {kebabExp && (
              <>
                <View style={s.kebabHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[s.kebabCatName, { color: theme.textPrimary }]}
                    >
                      {kebabExp.title}
                    </Text>
                    <Text style={[s.expMeta, { color: theme.textMuted }]}>
                      {fmtAmt(kebabExp.amount)}
                    </Text>
                  </View>
                </View>
                <View
          ...
          >
            ...
            {!isEdit && hasSavedDraft && showResumeBanner && (
              <View
                style={[
                  s.resumeBanner,
                  {
                    backgroundColor: theme.isDark ? "#1A3B2C" : "#EDF7F1",
                    borderColor: "#1B7A4A",
                  },
                ]}
              >
                <Ionicons name="bookmark-outline" size={15} color="#1B7A4A" />
                <Text style={[s.resumeText, { color: theme.textPrimary }]}>
            ...
            >
              <Text
                style={[
                  s.fieldTxt,
                  { flex: 1, color: vals.date ? theme.textPrimary : "#aaa" },
                ]}
              >
            ...
            >
              <Text
                style={[
                  s.fieldTxt,
                  { flex: 1, color: selectedCat ? theme.textPrimary : "#aaa" },
                ]}
              >
            ...
            >
              ...
              <Text style={[s.recurTxt, { color: theme.textPrimary }]}>
                Recurring Expense
              </Text>
            </TouchableOpacity>
          ...
          return (
            <>
              ...
              <Text
                style={[
                  pk.itemTxt,
                  { marginLeft: 12, color: theme.textPrimary },
                ]}
              >
                {cat.label}
              </Text>
...
export default function ManageExpenseScreen() {
  ...
  const [detailNavTab, setDetailNavTab] = useState<"Active" | "Archived">(
    "Active",
  );
  ...
  const loadData = useCallback(async (userId: string) => {
    try {
      ...
      setErrModal({
        ...
        message: err?.message ?? "Could not load expenses. Please try again.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const showConfirm = (opts: Omit<ConfirmState, "visible">) =>
    setConfirm({ visible: true, ...opts });
  const hideConfirm = () => setConfirm((prev) => ({ ...prev, visible: false }));
  const showError = (title: string, msg: string) =>
    setErrModal({ visible: true, title, message: msg });
    ...
    showConfirm({
      ...
      onYes: async () => {
        ...
        if (screen.name === "add") {
          ...
          const oldCatLbl = categories.find(
            (c) => c.id === oldExp?.categoryId,
          )?.label;
          const updatedFields = {
            categoryId: vals.categoryId,
            title: newTitle,
          ...
          setExpenses((prev) =>
            prev.map((e) =>
              e.id === screen.expenseId ? { ...e, ...updatedFields } : e,
            ),
          );
          sfx.success();
          ...
          if (error) {
            if (oldExp)
              setExpenses((prev) =>
                prev.map((e) => (e.id === screen.expenseId ? oldExp : e)),
              );
            ...
            if (oldExp) {
              ...
              if (oldExp.amount !== newAmount)
                changes.push(
                  `Amount: ${fmtAmt(oldExp.amount)} → ${fmtAmt(newAmount)}`,
                );
              if (oldExp.categoryId !== vals.categoryId)
                changes.push(
                  `Category: ${oldCatLbl ?? "?"} → ${catU?.label ?? "?"}`,
                );
              if (oldExp.date !== vals.date)
                changes.push(
                  `Date: ${fmtDateShort(oldExp.date)} → ${fmtDateShort(vals.date)}`,
                );
              if (oldExp.isRecurring !== vals.isRecurring)
                changes.push(
                  vals.isRecurring ? "Set to recurring" : "Recurring removed",
                );
              else if (
                oldExp.isRecurring &&
                oldExp.frequency !== vals.frequency
              )
                changes.push(
                  `Frequency: ${oldExp.frequency} → ${vals.frequency}`,
                );
            }
            logActivity({
              user_id: userIdRef.current!,
              ...
              description:
                changes.length > 0
                  ? changes.join(" · ")
                  : `${fmtAmt(newAmount)} · ${catU?.label ?? "Expense"}`,
  ...
  const handleSaveCategory = async (
    ...
    if (error) {
      if (oldCat)
        setCategories((prev) => prev.map((c) => (c.id === id ? oldCat : c)));
      ...
      logActivity({
        ...
        description:
          changes.length > 0 ? changes.join(" · ") : "Details updated",
        icon,
      });
    }
  };
    ...
    showConfirm({
      ...
      onYes: async () => {
        ...
        if (error) {
          ...
          logActivity({
            user_id: userIdRef.current!,
            action_type: ACTION.EXPENSE_CATEGORY_RESTORED,
            entity_type: ENTITY.EXPENSE_CATEGORY,
    ...
    showConfirm({
      ...
      onYes: async () => {
        ...
        if (error) {
          ...
          logActivity({
            user_id: userIdRef.current!,
            action_type: ACTION.EXPENSE_CATEGORY_DELETED,
            entity_type: ENTITY.EXPENSE_CATEGORY,
            title: `Category Deleted: ${cat?.label ?? "Expense Category"}`,
            description:
              "Expense category and all its entries permanently deleted.",
  ...
  const handleArchiveExpense = (expenseId: string) => {
    ...
    showConfirm({
      ...
      message: `Archive "${exp?.title ?? "this expense"}"?`,
      icon: "archive-outline",
      confirmLabel: "Archive",
      confirmColor: "#F59E0B",
      onYes: async () => {
        ...
        setExpenses((prev) =>
          prev.map((e) =>
            e.id === expenseId ? { ...e, isArchived: true } : e,
          ),
        );
        sfx.warning();
        ...
        if (error) {
          setExpenses((prev) =>
            prev.map((e) =>
              e.id === expenseId ? { ...e, isArchived: false } : e,
            ),
          );
          ...
          logActivity({
            user_id: userIdRef.current!,
            action_type: ACTION.EXPENSE_ARCHIVED,
            entity_type: ENTITY.EXPENSE,
  ...
  const handleRestoreExpense = (expenseId: string) => {
    ...
    showConfirm({
      ...
      message: `Restore "${exp?.title ?? "this expense"}"?`,
      icon: "refresh-outline",
      confirmLabel: "Restore",
      confirmColor: "#3ECBA8",
      onYes: async () => {
        ...
        if (error) {
          ...
          logActivity({
            user_id: userIdRef.current!,
            action_type: ACTION.EXPENSE_RESTORED,
            entity_type: ENTITY.EXPENSE,
    ...
    showConfirm({
      ...
      onYes: async () => {
        ...
        if (error) {
          ...
          logActivity({
            user_id: userIdRef.current!,
            action_type: ACTION.EXPENSE_DELETED,
            entity_type: ENTITY.EXPENSE,
  ...
  if (loading) {
    return (
      <SafeAreaView
        style={[s.safeArea, { backgroundColor: theme.headerBg }]}
      ...
      case "categories":
        return (
          <CategoriesScreen
            ...
            onEditCategory={(id) =>
              setEditCat(categories.find((c) => c.id === id) ?? null)
            }
          ...
          <ExpenseFormScreen
            ...
            onBack={() =>
              screen.prefillCategoryId
                ? setScreen({
                    name: "detail",
                    categoryId: screen.prefillCategoryId,
                  })
                : setScreen({ name: "categories" })
            }
            onSave={handleSave}
